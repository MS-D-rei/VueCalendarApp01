...
function highlightCurrentDate(num) {
  const currentFullDate = new Date(currentYear.value, currentMonth.value, num)
  return currentFullDate.toDateString() == new Date().toDateString() ? true : false
}
</script>
...
<template>
  <div class="m-auto">
    ...
    <section class="flex my-2">
      <p class="text-center" style="width: 14.28%" v-for="day of days" :key="day">
        {{ day }}
      </p>
    </section>
    <section class="flex flex-wrap">
      <p class="text-center" style="width: 14.28%" v-for="num in startDayIndex()" :key="num">
      </p>
      <p class="text-center" style="width: 14.28%" v-for="num in daysInMonth()" :key="num"
        :class="{ 'text-blue-600': highlightCurrentDate(num) }"> <!-- if num == today is true, class='text-blue-600' -->
        {{ num }}
      </p>
    </section>
